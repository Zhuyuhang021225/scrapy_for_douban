...
class WishSpider(CrawlSpider):
    name = "wish"
    user_id = input("输入你的豆瓣id:")
    allowed_domains = ["movie.douban.com"]
    start_urls = [
        "https://movie.douban.com/people/{}/wish?start=0&sort=time&rating=all&filter=all&mode=grid".format(user_id)]
    ...
    rules = (
        Rule(LinkExtractor(
            allow=r"/people/{}/wish\?start=\d+&sort=time&rating=all&filter=all&mode=grid".format(user_id)),
